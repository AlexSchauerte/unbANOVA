...
effects.unbANOVA <- function(g, reference.group = NULL){
  if(is.null(reference.group)){
    ...
    return(cbind(`Anova 1` = setNames(g$anova1$marginalMeans[-reference.group] - g$anova1$marginalMeans[reference.group], paste0(reference.group, " -> ", (1:length(g$anova1$marginalMeans))[-reference.group])),
                           `Anova 2` = setNames(g$anova2$marginalMeans[-reference.group] - g$anova2$marginalMeans[reference.group], paste0(reference.group, " -> ", (1:length(g$anova2$marginalMeans))[-reference.group])),
                           `Anova 3` = setNames(g$anova3$marginalMeans[-reference.group] - g$anova3$marginalMeans[reference.group], paste0(reference.group, " -> ", (1:length(g$anova3$marginalMeans))[-reference.group])),
                           `ATE`     = setNames(g$ATE$marginalMeans[-reference.group] - g$ATE$marginalMeans[reference.group], paste0(reference.group, " -> ", (1:length(g$ATE$marginalMeans))[-reference.group]))))
  } else {
